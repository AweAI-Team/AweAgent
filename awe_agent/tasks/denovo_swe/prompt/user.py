...
# ── DeNovoSWE: Search prompt ─────────────────────────────────────────────────


SEARCH_DENOVOSWE_DOC2REPO_PROMPT = """I need you to implement a software repository from scratch based on a strict architectural specification.
...
# ── DeNovoSWE v2: Search prompt (with public API verification gate) ─────────

SEARCH_DENOVOSWE_DOC2REPO_PROMPT_V2 = """I need you to implement a software repository from scratch based on a strict architectural specification.
...
**Phase 4: Submission**
* **No git operations needed — you don't need to perform any git operations.** The evaluator captures your file changes automatically by diffing the workspace against the baseline commit when you call `finish`. Do **NOT** run any git command (`git add`, `git commit`, `git config user.*`, `git status`, `git diff`, `git checkout`, etc.) — manual git operations have **zero** effect on your score and only waste turns.
* Once the verification passes and you are confident:
    1. Delete any temporary verification scripts (`/tmp/verify_public_api.py`, `tests/`) to keep the repo clean.
    2. Ensure `setup.py` is configured correctly according to <DEPENDENCY_RULES>.
    3. Submit your work using the "finish" tool.
...
USER_PROMPTS: dict[str, str] = {
    ...
    "search_denovoswe_doc2repo":     SEARCH_DENOVOSWE_DOC2REPO_PROMPT,
    "denovoswe_doc2repo_v2":         DENOVOSWE_DOC2REPO_PROMPT_V2,
    "search_denovoswe_doc2repo_v2":  SEARCH_DENOVOSWE_DOC2REPO_PROMPT_V2,
}
